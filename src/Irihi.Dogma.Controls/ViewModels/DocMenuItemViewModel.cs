...
/// 标题经 <see cref="ILinguaManager"/> 按 TitleKey 解析；未提供 manager 或键缺失时回退 FallbackTitle 字面量。
/// </summary>
public partial class DocMenuItemViewModel : ObservableObject
{
    ...
    public DocMenuItemViewModel(DocCategoryNode node, ILinguaManager? manager = null)
    {
        var titleKey = node.Page?.Metadata.TitleKey;
        MenuHeader = titleKey is null
            ? null
            : manager?.GetObservable(titleKey) ??
