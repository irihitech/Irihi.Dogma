...
public sealed class DocPageGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var projectNamespace = context.AnalyzerConfigOptionsProvider
            .Combine(context.CompilationProvider)
            .Select(static (source, _) =>
                source.Left.GlobalOptions.TryGetValue("build_property.RootNamespace", out var rootNamespace)
                    ? rootNamespace
                    : source.Right.AssemblyName);
        var combined = categories.Collect().Combine(pages.Collect()).Combine(projectNamespace);

        context.RegisterSourceOutput(combined,
            static (spc, source) => Emit(source.Left.Left, source.Left.Right, source.Right, spc));
    }
        ...
        SourceProductionContext spc)
    {
        ...
        {
            sb.Append("namespace ").Append(projectNamespace).AppendLine(";");
            sb.AppendLine();
        }
